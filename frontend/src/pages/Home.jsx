...
export default function Home() {
  ...
  const [pendingFriendsCount, setPendingFriendsCount] = useState(() => {
    return parseInt(localStorage.getItem('studyconect_pending_friends') || '0', 10);
  });
  ...
  const fetchSideData = useCallback(async () => {
    if (!user?.id) return;
    try {
      // 1. Get unread chats count (synchronous from localStorage, no network wait!)
      const unreadCount = getTotalUnread(user.id);
      setChatUnread(unreadCount);
      localStorage.setItem('studyconect_chat_unread', unreadCount.toString());

      // 2. Execute the async network calls in parallel using Promise.all
      const [friendshipsRes, schedulesRes] = await Promise.all([
        supabase
          .from('friendships')
      ...
      // 3. Friends pending requests count (only update if successfully fetched)
      if (friendshipsRes && friendshipsRes.count !== null) {
        const pCount = friendshipsRes.count || 0;
        setPendingFriendsCount(pCount);
        localStorage.setItem('studyconect_pending_friends', pCount.toString());
      }
