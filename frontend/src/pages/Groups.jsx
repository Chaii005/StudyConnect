            ...
            <div key={`${meetingMode}-${step}`} style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', flex: 1 }}>
                    ...
                    {subjectDropdownOpen && (
                      ...
                      >
                        <div
                          ...
                          onMouseLeave={e => { e.currentTarget.style.background = !formData.subject && subjectMode === 'select' ? 'var(--bg-input)' : 'transparent'; e.currentTarget.style.color = !formData.subject && subjectMode === 'select' ? 'var(--text-primary)' : 'var(--text-secondary)'; }}
                        >
                          -- Chọn môn học --
                        </div>

                        {dbSubjects.map(s => {
                          const isSelected = subjectMode === 'select' && formData.subject === s;
                          return (
                            <div
                              key={s}
                              onClick={() => {
                        ...
                        {!userMajor && (
                          <div
                            onClick={() => {
                              setSubjectMode('select');
                              setFormData({ ...formData, subject: 'Chung' });
                              setSubjectDropdownOpen(false);
                            }}
                            style={{
                              padding: '8px 12px',
                            ...
                            onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-input)'; e.currentTarget.style.color = 'var(--text-primary)'; }}
                            onMouseLeave={e => { e.currentTarget.style.background = formData.subject === 'Chung' && subjectMode === 'select' ? 'var(--bg-input)' : 'transparent'; e.currentTarget.style.color = formData.subject === 'Chung' && subjectMode === 'select' ? 'var(--text-primary)' : 'var(--text-secondary)'; }}
                          >
                            Chung
                          </div>
                        )}
              ...
              <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <div className="form-input-wrap">
                  <div style={{ position: 'relative' }}>
                    <div 
                      onClick={() => setMaxMembersDropdownOpen(!maxMembersDropdownOpen)}
                      style={{
                        width: '100%',
                        background: 'var(--bg-input)',
                        border: '1px solid var(--border)',
                    ...
                    >
                      <span>{formData.maxMembers || 10} thành viên</span>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" style={{ transform: maxMembersDropdownOpen ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s' }}>
                        <polyline points="6 9 12 15 18 9" />
                      </svg>
                    </div>

                    {maxMembersDropdownOpen && (
                      <div 
                        style={{
                          position: 'absolute',
                          top: '100%',
                      ...
                      >
                        {[5, 10, 15, 20, 30, 50, 100].map(val => {
                          const isSelected = (formData.maxMembers || 10) === val;
                          return (
                            <div
                              key={val}
                              onClick={() => {
                                setFormData({ ...formData, maxMembers: val });
...
// ── CUSTOM PROMPT MODAL ───────────────────────────────────────────────────
function PromptModal({ isOpen, title, message, defaultValue, placeholder, onConfirm, onCancel }) {
...
// ── CUSTOM SUBJECT SELECT MODAL ───────────────────────────────────────────
function SubjectSelectModal({ isOpen, title, userMajor, defaultValue, onConfirm, onCancel }) {
  const [dbSubjects, setDbSubjects] = useState([]);
  const [subjectMode, setSubjectMode] = useState('select'); // 'select' | 'custom'
  ...
  useEffect(() => {
    if (isOpen) {
      const isKnown = dbSubjects.includes(defaultValue) || (!userMajor && defaultValue === 'Chung');
      if (isKnown || defaultValue === '') {
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setSubjectMode('select');
        ...
        setCustomSubject(defaultValue || '');
      }
      setDropdownOpen(false);
    }
  }, [isOpen, defaultValue, dbSubjects, userMajor]);

  if (!isOpen) return null;

  const handleSubmit = (e) => {
    e.preventDefault();
    const finalSubject = subjectMode === 'custom' ? customSubject.trim() : selectedSubject;
    if (!finalSubject) return;
    onConfirm(finalSubject);
  };

  const options = [...dbSubjects];
  if (!userMajor) {
    options.push('Chung');
  }
  options.push('custom');
  ...
  const getLabel = (val) => {
    if (val === 'custom') return 'Môn học khác...';
    if (!val) return '-- Chọn môn học --';
    return val;
  };
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, position: 'relative' }}>
          ...
          >
            <span>{getLabel(subjectMode === 'custom' ? 'custom' : selectedSubject)}</span>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" style={{ transform: dropdownOpen ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 0.2s' }}>
              <polyline points="6 9 12 15 18 9" />
            </svg>
          </div>
            ...
            >
              <div
                ...
                style={{
                  ...
                  color: 'var(--text-secondary)',
                  transition: 'all 0.15s',
                  background: selectedSubject === '' && subjectMode === 'select' ? 'var(--bg-input)' : 'transparent',
                }}
                onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-input)'; e.currentTarget.style.color = 'var(--text-primary)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = selectedSubject === '' && subjectMode === 'select' ? 'var(--bg-input)' : 'transparent'; e.currentTarget.style.color = selectedSubject === '' && subjectMode === 'select' ? 'var(--text-primary)' : 'var(--text-secondary)'; }}
              >
                -- Chọn môn học --
              </div>

              {options.map(opt => {
                const isSelected = (opt === 'custom' && subjectMode === 'custom') || (opt !== 'custom' && subjectMode === 'select' && selectedSubject === opt);
                return (
                  <div
                    key={opt}
                    onClick={() => {
...
export default function Groups() {
  ...
  return (
    ...
    <SubjectSelectModal
      isOpen={!!subjectSelectConfig}
      title={subjectSelectConfig?.title}
      userMajor={subjectSelectConfig?.userMajor}
      defaultValue={subjectSelectConfig?.defaultValue}
      onConfirm={subjectSelectConfig?.onConfirm}
      onCancel={() => setSubjectSelectConfig(null)}
    />
  </>
);
}